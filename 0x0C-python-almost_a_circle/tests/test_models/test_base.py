...
class TestBase(unittest.TestCase):
	...
	self.assetTrue(isinstance(b6, Base))
	...
	self.assertTrue(isinstance(json_d, "[]")
	...
	self.assertEqual(
		"list dictionaries must be a list of dictionaries", str(x.exception))
	with self.assertRaises(TypeError) as x:
	...
	def test_16_1(self):
		...
		with self.assertRaises(AttributeError) as x:
	        ...
	        self.assertEqual(
			"Base' object has no attribute 'to_dictonary'", str(
			x.exception))
		with self.assertRaises(AttributeError) as x:
                        Base.save_to_file([3, 4])
                self.assertEqual(
                        "Base' object has no attribute 'to_dctionary'", str(
                        x.exception))
		with self.assertRaises(TypeError) as x:
			Rectangle.save_to_files(5)
		self.assertEqual(
	...
	def test_16_2(self):
		...
		with self.assertRaises(TypeError) as x:
			Rectangle.save_to_file()
		self.assertEqual(s1, str(x.execption))
	...
	def test_17_1(self);
		"""Test static method from _json_string with wrong types"""
	...
	def test_17_2(self):
		"""tests static method from_json_string with wrong args"""
	...
	def test_18_0(self):
		...
		self.assertEqual(r1  is r2)
		self.assertEqual(r1 == r2)
		s1 = Square(3, 5)
		s1_dictionary = s1.to_dictionary()
		s2 = Square.create(**s1_dictionary)
		self.assertEqual(str(s1), str(s2))
		self.assertEqual(s1 is s2)
		self.assertFalse(s1 == s2)

	def test_18_1(self):
		"""test class mehod create with wrong types"""
	...
	def test_20_1(self):
		...
		self.assertEqual(
			"list_objs ust be a list of instances", str(
				x.exception))
		with self.assertRaises(TypeError) as x:
	...
	def test_20_2(self):
		...
		self.assertEqual(s1, str(x.exception))
		s2 = "save_to_file_csv() takes 2 positioanl arguments but 3 were given"
		with self.assertRaises(TypeError) as x:
	...
	def test_20_3(self):
		...
		r2 = Rectangle(2, 4)
		list_rectangles_input = [r1, r2]
		Rectangle.save_to_filele_csv(list_rectangles_input)
