...
class Base:
	...
	def to_json_string(list_dictionaries):
		...
		if (type(list_dictionaries) != list or
			not all(type(x) == dict for x in list_dictionaries)):
				raise TypeError("list_objs must be a list ofinstances")		
	...
	def from_json_string(json_string):
		...
		if json_string is not None and json_string != '':
			if type(json_string) != str:
				raise TypeError("json_string must be astring")
	...
	@classmethod
	def create(cls, **dictionary):
		"""Returns an instance with all attributes already set."""4
		if cls.__name__ == 'Rectangle':
			dummy = cls(1, 1)
		elif cls.__name__ == 'Square':
			dummy = cls(1)
	...
	def save_to_file_csv(cls, list_objs):
		"""Serialises list_objs in CSV format and saves it to a file."""
			
		if(type(list_objs) != list and 
		list_objs is not None or
		not all(isinstance(x, cls) for x in list_objs))
			raise TypeError("list_objs must be list of instances")
	...
	def load_from_file_csv(cls):
		"""A method that serialises and deserialises a list of instances"""
	...
	def draw(list_rectangles, list_squares):
		"""A static method that opends a window and draws all the instances."""
